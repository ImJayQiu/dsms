...
class CdoanalysisesController < ApplicationController
	...
	def season
		...
		@mean_min = [] 
		@seasmin.each do |i|
			@min_min << (i.split(" ")[8].to_i * @rate).to_i
			@mean_min << (i.split(" ")[9].to_i * @rate).to_i
			@max_min << (i.split(" ")[10].to_i * @rate).to_i
		end 
		...
		@mean_max = [] 
		@seasmax.each do |i|
			@min_max << (i.split(" ")[8].to_i * @rate).to_i
			@mean_max << (i.split(" ")[9].to_i * @rate).to_i
			@max_max << (i.split(" ")[10].to_i * @rate).to_i
		end 
	...
	def yearly
		...
		@ymean_min = [] 
		@yearmin.each do |i|
			@ymin_min << (i.split(" ")[8].to_i * @rate).to_i
			@ymean_min << (i.split(" ")[9].to_i * @rate).to_i
			@ymax_min << (i.split(" ")[10].to_i * @rate).to_i
		end 
		...
		@ymean_mean = [] 
		@yearmean.each do |i|
			@ymin_mean << (i.split(" ")[8].to_i * @rate).to_i
			@ymean_mean << (i.split(" ")[9].to_i * @rate).to_i
			@ymax_mean << (i.split(" ")[10].to_i * @rate).to_i
		end 
		...
		@ymean_max = [] 
		@yearmax.each do |i|
			@ymin_max << (i.split(" ")[8].to_i * @rate).to_i
			@ymean_max << (i.split(" ")[9].to_i * @rate).to_i
			@ymax_max << (i.split(" ")[10].to_i * @rate).to_i
		end 

...
class Cmip5sController < ApplicationController
	...
	skip_before_filter :authenticate_user!, :except => [:daily_analysis, :mult_analysis]
	...
	def daily_analysis
		...
		@sel_data = cdo_run.seldate([@sdate.to_datetime, @edate.to_datetime], input: cdo_run.sellonlatbox([s_lon,e_lon,s_lat,e_lat], input: file), output: "public/#{@cdo_output_path}.nc", options: '-f nc4') rescue nil
		##############################################################

		if @sel_data.nil?

			@status = "No data available !<br/>Please check your selection and try again !"
	...
	def mult_analysis
		...
		exp = params[:exp].first.to_s rescue nil

		exp1 = params[:exp1].first.to_s rescue nil
		exp2 = params[:exp2].first.to_s rescue nil
		exp3 = params[:exp3].first.to_s rescue nil
		exp4 = params[:exp4].first.to_s rescue nil
		...
		@exp1_path = Settings::Experiment.where(name: exp1).first.name rescue nil
		@exp2_path = Settings::Experiment.where(name: exp2).first.name rescue nil
		@exp3_path = Settings::Experiment.where(name: exp3).first.name rescue nil
		@exp4_path = Settings::Experiment.where(name: exp4).first.name rescue nil

...
class NexnasaController < ApplicationController
	...
	def daily_analysis
		...
		cdo = Cdo.new(debug: true, logging: true, logFile: 'cdo_commands_nexnasa.log')
		...
		@file_name = var + '_day_BCSD_' + experiment + '_r1i1p1_' + model + '_' + 'rimes' + '.nc'
		...
		@model_path = Settings::NexnasaModel.where(name: model).first.folder
